...
- 2026-05-23：新增 YOLO 模型训练功能，支持超参数配置、GPU/CPU 训练、迁移学习。
- 2026-05-23：重构 UI 为 .ui + pyside6-uic 模式、采用 .qrc 资源系统管理图标。
...
- **🎯 YOLO 模型训练**：内置训练对话框，支持数据集校验、模型下载、GPU/CPU 训练
- **🔄 迁移学习**：支持加载预训练模型或之前训练的检查点继续训练
- **⚙️ 完整训练控制**：轮数、批量、学习率、优化器（SGD/Adam/AdamW）、数据增强（mosaic、mixup、HSV 抖动、几何变换）
...
7. **YOLO 模型训练**：点击左侧工具栏的”模型训练”，选择数据集目录（需包含 data.yaml），配置超参数后点击”开始训练”。训练在独立进程中运行，实时输出日志和进度。
